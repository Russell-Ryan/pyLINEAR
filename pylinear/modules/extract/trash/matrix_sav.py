...
from .lsqrresult import LSQRResult
...
class Matrix(object):
    ...
    def __init__(self,grisms,sources,extbeams,mskbeams=None,path='tables',
        ...
        if usehdf5:
            i,j,aij,self.bi=self.load_from_hdf5(grisms,sources,extbeams,mskbeams,hdf5file)
        else:
            i,j,aij,self.bi=self.load_from_images(grisms,sources,extbeams,mskbeams,hdf5file)
        ...
        with h5table.H5Table(dataset,path=self.path,mode='r') as h5tab:
            for device in grism:
                ...
                for kk,source in enumerate(sources):
                    ...
                    ddt=self.load_ddts(source,device,h5tab,config)
    ...
    def load_ddts(self,source,device,h5tab,config,threshold=None):
    ...
    def run_lsmr(self,logdamp,scale=True,**kwargs):
        ...
        t1=default_timer()
        r=ssl.lsmr(self.A,self.bi,damp=damp,calc_var=True,**kwargs)
